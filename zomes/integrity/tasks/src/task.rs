...
pub fn validate_status_from_dependencies(
    ...
    let non_optional_dependencies: Vec<TaskDependency> = dependencies
        .into_iter()
        .filter(|d| !d.optional)
        .collect();
    match task_status {
        ...
        TaskStatus::Ready | TaskStatus::InProgress | TaskStatus::Done => {
            if non_optional_dependencies.iter().all(|d| d.status.eq(&TaskStatus::Done)) {
        ...
        TaskStatus::Blocked => {
            if non_optional_dependencies.iter().all(|d| d.status.eq(&TaskStatus::Done)) {
                return Ok(
                    ValidateCallbackResult::Invalid(
                        String::from("All tasks are done: this task is not blocked."),
                    ),
                );
            } else {
                return Ok(ValidateCallbackResult::Valid);
...
pub fn validate_create_task(
    ...
    match action {
        EntryCreationAction::Create(_) => {
            if let Some(_) = task.original_create_hash {
                return Ok(
                    ValidateCallbackResult::Invalid(
                        String::from("Create tasks can't have original_action_hash"),
                    ),
                );
            }
        }
        EntryCreationAction::Update(_update) => {
            if let None = task.original_create_hash {
                return Ok(
                    ValidateCallbackResult::Invalid(
                        String::from("Update tasks must have original_action_hash"),
                    ),
                );
            }
        }
    ...
    for dependency in task.dependencies.clone() {
        ...
        if !last_revision_dependency_task.status.eq(&dependency.status) {
            return Ok(
                ValidateCallbackResult::Invalid(
                    String::from("Invalid TaskDependency status"),
                ),
            );
        }
    }
...
pub fn validate_update_task(
    ...
    if let Some(original_create_hash) = task.original_create_hash {
        ...
        return Ok(
            ValidateCallbackResult::Invalid(
                String::from("Task update must have original_create_hash"),
            ),
        );
    }
...
pub fn validate_delete_link_task_updates(
    ...
    Ok(
        ValidateCallbackResult::Invalid(
            String::from("TaskUpdates links cannot be deleted"),
        ),
    )
}
